...
sf::Vector2f screen_wrap(const sf::Vector2f &position) {
  ...
  } else if (wrapped_position.x > static_cast<float>(DISPLAY_SIZE.x)) {
    wrapped_position.x = 1.0F;
  }
  if (wrapped_position.y < 0.0F) {
    wrapped_position.y = static_cast<float>(DISPLAY_SIZE.y - 1U);
  } else if (wrapped_position.y > static_cast<float>(DISPLAY_SIZE.y)) {
    wrapped_position.y = 1.0F;
  }
